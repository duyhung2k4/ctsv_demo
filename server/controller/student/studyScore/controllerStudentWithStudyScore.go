package studyscore

import (
	"ctsv/model"
	getdataStudyScore "ctsv/repository/getdata/studyScore"
	"ctsv/statusrequest"
	"encoding/json"
	"net/http"

	"github.com/go-chi/jwtauth/v5"
)

// @Summary
// @Description Student get study score
// @Tags Student with study score
// @Accept json
// @Produce json
// @Router /student/study_score [get]
func StudentGetStudyScore(w http.ResponseWriter, r *http.Request) {

	_, claims, err := jwtauth.FromContext(r.Context())

	if err != nil {
		statusrequest.ErrorRequest(w, r, err, "", "400")
	} else {
		baseInfoId := int(claims["base_info_id"].(float64))
		listStudyScore, err := getdataStudyScore.GetStudyScoreWithBaseInfoId(baseInfoId)

		if err != nil {
			statusrequest.ErrorRequest(w, r, err, "", "400")
		} else {
			result, _ := json.Marshal(model.Request{
				Data: model.DataRequest{
					ListStudyScore: listStudyScore,
				},
				Message: "",
				Error:   nil,
				Success: true,
			})

			w.WriteHeader(http.StatusOK)
			w.Write(result)
		}
	}

}
